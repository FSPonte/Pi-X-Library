...
constexpr static unsigned long
	PROGRESS_BAR_WIDTH_DEFAULT = 40, // Progress bar default width
	TERMINAL_WIDTH_DEFAULT = 80, // Terminal width when it cannot be determined
	FILE_WIDTH_DEFAULT = 120; // Default output width when printing to file
...
namespace pix
{
	...
	{
		const utf8_codes codes = get_utf8_codes();
		std::cout << "\r";

		if (codes.is_utf8)
		{
			std::cout << codes.reset;
			std::cout << codes.disable_cursor;
			std::cout << codes.erase_current_line;
		}
		...
		const char* fill_color = this->_is_finished ? codes.color_fill_after_ended : codes.color_fill;

		// Spinner
		if (codes.spinner_animation_length > 0)
		{
			const int spinner_index = this->_internal.updates_count % codes.spinner_animation_length;
				
			std::cout << codes.color_spinner;
			std::cout << codes.spinner[spinner_index];
			std::cout << codes.reset;
			std::cout << " ";
		}
		...
		if (filled_half_cells > 0)
		{
			...
			for (int i = 0; i < full_cells; i++)
				std::cout << codes.bar_fill;

			if (has_left_half_cell)
				std::cout << codes.bar_fill_head;
		...
		if (empty_cells > 0)
		{
			std::cout << codes.color_empty;

			if (has_right_half_cell)
				std::cout << codes.bar_empty_head;

			int i = (has_left_half_cell || has_right_half_cell) ? 1 : 0;
			
			for (; i < empty_cells; ++i)
				std::cout << codes.bar_empty;
		}
		
		std::cout << codes.reset;
		std::cout << codes.bar_suffix;

		// Extra Info
		std::printf(
			" %s%3d%%%s %s ",
			codes.color_percentage,
			static_cast<int>(clamped),
			codes.reset,
			codes.separator
		);
		
		std::cout << codes.color_elapsed_time;
		this->print_elapsed_time();
		std::cout << codes.reset;
		std::cout << " ";
		std::cout << codes.separator;
		std::cout << " ";
		std::cout << codes.color_remaining_time;
